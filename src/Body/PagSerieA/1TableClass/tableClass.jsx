...
const TableClass = () => {
  ...
  return (
    <>
      {isBigScreen && (
        <table className="relative overflow-auto min-h-[57rem] md:h-auto [60.7rem] lg:h-[57rem]">
          <thead>
          ...
          <tbody className="bg-black text-cyan-800">
            {squadreOrdinate.map((squadra, index) => (
              ...
              <tr key={index}>
                ...
                {/* { COLONNA SQUADRE} */}
                <td
                  className={`w-[100%] bg-black xs:pl-[0] sm:pl-[1rem] md:pl-[1.5rem] md:pr-[2rem] py-[4px] md:py-2 sm:text-lg md:text-md lg:text-lg xl:px-4 flex justify-start relative sq-column
            ...
            ${isDrawingTeamInCoppiaRegSelected(squadra.name) ? `${s.Filter4} underlineX ${isCoppiaSelected(squadra.name) ? "" : s.Bg3}` : ""}          
              ...
              `}
                ...
                >
                  <div className="absolute transform -translate-x-4/3 -translate-y-5 text-left text-lg text-white mx-8 my-[-10] z-[10]">{numeriIndiciBorderWhite[index]}</div>
              ...
              ${isDrawingTeamInCoppiaRegSelected(squadra.name) ? `${s.Filter3} ${s.BaseText} ${isCoppiaSelected(squadra.name) ? `${s.BaseText}` : `${s.Bg3}`}` : ""} 
                ...
                >
                  ...
                  <div className={`absolute transform -translate-x-4/3 -translate-y-5 text-center text-md text-gray-600/80 mx-8 my-[-10] z-30`}>
      ...
      {isMobile && (
        <div className="table-container h-[43.5vh] max-h-[44vh] ">
              ...
              <tbody className="bg-black text-cyan-800" style={{ position: "relative", top: "-1.5rem", zIndex: 1 }}>
                {firstHalf.map((squadra, index) => (
                  <tr key={index}>
                    ...
                    <td
                      className={`w-[100%] bg-black py-[4px] pl-0 sm:pl-[1rem] md:pl-[1.5rem] md:pr-[2rem] md:py-2 sm:text-lg md:text-md lg:text-lg xl:px-4 flex justify-start relative sq-column
                      ${isCoppiaSelected(squadra.name) ? `${s.Bg2} ${s.Filter2} ${s.BaseText1}` : ""}
                      ${isTeamMarkedWithX(squadra.name) ? `${s.Filter4} underlineX ` : sqSelected.includes(squadra.name + "Z") ? `${s.Filter4} underlineW` : sqSelected.includes(squadra.name + "Y") ? `${s.Filter4} underlineL` : ""}
                      ${isWinningTeamInCoppiaRegSelected(squadra.name) ? `${s.Filter4} underlineW ${isCoppiaSelected(squadra.name) ? "" : s.Bg3}` : ""}
                      ${isLosingTeamInCoppiaRegSelected(squadra.name) ? `${s.Filter4} underlineL ${isCoppiaSelected(squadra.name) ? "" : s.Bg3}` : ""}
                    ...
                    >
                      ...
                      <div className="absolute transform -translate-x-4/3 -translate-y-5 sm:-translate-y-6 md:-translate-y-8 text-center md:text-lg text-md text-yellow-500 mx-2 sm:mx-3 md:mx-0 lg:mx-2 my-[-10] z-[10]">
                    ...
                    >
                      <div className="innerBorder"></div>
                      <div className={`absolute transform -translate-x-4/3 -translate-y-5 text-left text-md text-white mx-2 sm:mx-8 sm:my-[-10] !z-50`}>
                        {!numeriIndiciBorderWhite[index] && differenzePunti[index]}
                      </div>
                      {getPunteggioColonnaPTS(squadra)}
                    </td>
