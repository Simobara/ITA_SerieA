...
const Partite = ({ resetAll, occhioApertoPartita, setOcchioApertoPartita }) => {
  ...
  return (
    ...
    <div
      className={`relative overflow-x-hidden overflow-y-scroll hide-scrollbar flex flex-col items-left xs:text-xs sm:text-xl border border-x-0 rounded-3xl ${s.BorderPartite} py-[5%] min-w-[17rem] max-h-[26rem]`}
    >
      {giorniSettimana.map(
        ({ giorno, title }) =>
