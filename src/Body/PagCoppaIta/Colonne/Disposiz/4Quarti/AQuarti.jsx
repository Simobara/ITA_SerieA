...
const AQuarti = ({ width = '70px', height = '24px' }) => {
    ...
    const ENDPOINTS = {
        quartiA1: '/api/coppaItaQuartiA1/quartiA1',
        quartiA2: '/api/coppaItaQuartiA2/quartiA2'
    };
    ...
    const handleSaveTeamName = (newTeamName) => {
        ...
        const endpoint = stage === 'quartoA1' ? ENDPOINTS.quartiA1 : ENDPOINTS.quartiA2;
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const responseA1 = await axios.get(`${API_BASE_URL}${ENDPOINTS.quartiA1}`);
                const responseA2 = await axios.get(`${API_BASE_URL}${ENDPOINTS.quartiA2}`);
