...
export function generateMonthDays(year, month) {
  ...
  const firstDayOfMonth = new Date(
    year,
    monthIndex < 5 ? monthIndex + 7 : monthIndex - 5,
    1
  );
  const daysInMonth = new Date(
    year,
    monthIndex < 5 ? monthIndex + 8 : monthIndex - 4,
    0
  ).getDate();
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    const dayNameCapitalized =
      dayName.charAt(0).toUpperCase() + dayName.slice(1).toLowerCase(); // Capitalizza la stringa
